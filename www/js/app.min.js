...
rxControllers.controller("compareCtrl", [ "$scope", "$routeParams", "restaurantSvr", function(a, b, c) {
    a.restaurantId = b.restaurantId;
    c.getRestaurant(a.restaurantId).then(function(b) {
        a.restaurant = b;
        a.restaurants = [];
        a.addRestaurant(b);
    });
    ...
    a.removeGraph = function(b) {
        var c = a.flotDataset;
        for (var d = 0; d < c.length; d++) {
            if (c[d].restaurantId == b.id) {
                c.splice(d, 1);
                return;
            }
        }
    ...
    a.addGraph = function(b) {
        console.log(b);
        c.getGraphs(b.id, "OVERALL").then(function(c) {
            a.flotDataset.push({
                data: c.trend,
                label: " " + b.name,
                restaurantId: b.id,
...
angular.module("directive", [ "restaurantService" ]).directive("searchBox", [ "restaurantSvr", searchBox ]);
...
function searchBox(a) {
    return {
        ...
        replace: true,
        link: function(b) {
            b.navSearch = function(b) {
                return a.findRestaurant(b).then(function(a) {
                    if (!a.length) {
                        a.push({
