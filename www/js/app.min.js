...
rxControllers.config([ "$routeProvider", function(a) {
...
}).controller("indexCtrl", [ "$scope", "$http", "localStorageService", "$location", "restaurantSvr", "geoLocation", "$routeParams", "searchData", function(a, b, c, d, e, f, g, h) {
    a.restaurantList = {
        params: {
            sort: "popular"
        }
    ...
    j();
    if (g.search) {
        a.restaurantList.params = merge_objects(a.restaurantList.params, {
            "formatted-address": g.formattedAddress,
            price_range: g.priceRange,
            category: ""
        });
        i();
    }
    if (!c.get("latitude") || !c.get("longitude")) {
        f.getLocation().then(function(a) {
            c.add("latitude", a.coords.latitude);
            c.add("longitude", a.coords.longitude);
            i();
        });
        i();
    } else {
    ...
    a.popularListPageChanged = function() {
        var b = a.popularListCurrentPage;
        i({
            page: b
        });
    };
    ...
    function i(b) {
        ...
        e.getRestaurants(b).then(function(b) {
            a.restaurants = b.items;
            a.maxSize = 6;
            a.popularListItemPerPage = 8;
            a.popularListTotalItems = b._meta.totalCount;
            a.popularListCurrentPage = b._meta.currentPage + 1;
        });
    }
    a.trend = [ {
        key: "Trend",
        values: [ [ 10254096e5, 0 ], [ 1028088e6, 6.3382185140371 ], [ 10307664e5, 5.9507873460847 ], [ 10333584e5, 11.569146943813 ], [ 10360404e5, 5.4767332317425 ], [ 10386324e5, .50794682203014 ], [ 10413108e5, 5.5310285460542 ], [ 10439892e5, 5.7838296963382 ], [ 10464084e5, 7.3249341615649 ], [ 10490868e5, 6.7078630712489 ], [ 13304916e5, 13.388148670744 ] ]
    } ];
    a.options = {
        animate: {
            duration: 1e3,
...
} ]).controller("navigationController", [ "$scope", "$http", "$location", "$rootScope", "restaurantSvr", "localStorageService", function(a, b, c, d, e, f) {
    a.user = f.get("user");
    a.navSearch = function(a) {
        return e.findRestaurant(a).then(function(a) {
            if (!a.length) {
                a.push({
    ...
    a.setRestaurant = function(b) {
        if (Object.keys(b).length) {
            c.path("/restaurant/" + b.data.id);
            a.searchRestaurant = "";
        }
    };
    ...
    a.searchRestaurant = function() {
        f.set(a.search);
        c.url("/index?search=true&formattedAddress=" + a.search.formattedAddress + "&priceRange=" + a.search.price + "&category=" + a.search.category);
    };
} ]).controller("mapCtrl", [ "$scope", "locationSvr", "$modal", "$routeParams", "$log", function(a, b, c, d, e) {
    a.getMap = function() {
...
rxControllers.controller("detailCtrl", [ "$scope", "$timeout", "$upload", "localStorageService", "$location", "$routeParams", "restaurantSvr", "geoLocation", "reviewSvr", "overviewSvr", "locationSvr", "photoSvr", "$anchorScroll", "$modal", function(a, b, c, d, e, f, g, h, i, j, k, l, m, n) {
    var o = null;
    u();
    m();
    var p = d.get("user");
    ...
    function q() {
        ...
        if (p !== null) a.activationForm.user_id = p.id;
    }
    ...
    var o = r(f.request);
    if (typeof o !== undefined && Object.keys(o).length && p !== null) {
        o = n.open({
            templateUrl: o.templateLocation,
            scope: a,
    ...
    function v(b) {
        j.getGraph(b).then(function(b) {
            a.lineChartYData = [ {
                name: "Restaurant Overview",
                data: b
            } ];
            a.lineChartXData = [ "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec" ];
        });
    }
} ]);
...
rxControllers.controller("graphCtrl", [ "$scope", function(a) {
    ...
    a.chartConfig = {
        ...
        subtitle: {
            text: "From " + b.min_dt + " to " + b.max_dt
        },
        xAxis: {
            type: "datetime"
        },
    ...
    var c = [ [ 0, 0 ], [ 1, 0 ], [ 2, 1 ], [ 3, 2 ], [ 4, 15 ], [ 5, 5 ], [ 6, 12 ], [ 7, 10 ], [ 8, 55 ], [ 9, 13 ], [ 10, 25 ], [ 11, 10 ], [ 12, 12 ], [ 13, 6 ], [ 14, 2 ], [ 15, 0 ], [ 16, 0 ] ];
    a.flotDataset = [ c ];
    a.flotOptions = {
        series: {
            lines: {
...
rxControllers.controller("reviewCtrl", [ "$scope", "localStorageService", "$routeParams", "reviewSvr", "$modal", "messageCenterService", "$timeout", function(a, b, c, d, e, f, g) {
    ...
    function k(b) {
        d.getRestaurantReviews(c.restaurantId, b).then(function(b) {
            ...
            a.reviewListCurrentPage = b._meta.currentPage + 1;
        });
    }
} ]);
...
rxControllers.controller("photoCtrl", [ "$scope", "$routeParams", "photoSvr", "messageCenterService", "$upload", function(a, b, c, d, e) {
    var f = b.restaurantId;
    a.dynamic = "";
    a.type = "";
    g();
    function g(b) {
        ...
        c.getRestaurantPhotos(f, b).then(function(b) {
            a.photos = b.items;
            a.maxSize = 6;
            a.photosListItemPerPage = 8;
            a.photosListTotalItems = b._meta.totalCount;
            a.photosListCurrentPage = b._meta.currentPage + 1;
        });
    }
    ...
    a.deletePhoto = function(b) {
        var d = a.photos[b].id;
        c.deleteRestaurantPhotos(d).then(function(a) {});
    };
    a.$watch("restaurantPhotos", function() {
        a.type = "warning";
        for (var c = 0; c < a.restaurantPhotos.length; c++) {
            var d = a.restaurantPhotos[c];
            a.upload = e.upload({
                url: "http://api.iresturant.com/v1/photos/upload?access-token=f899139df5e1059396431415e770c6dd",
                method: "POST",
                data: {
                ...
                a.type = "info";
                a.dynamic = parseInt(100 * b.loaded / b.total);
...
rxControllers.controller("claimCtrl", [ "$scope", "$modal", "$routeParams", "$timeout", "messageCenterService", "claimSvr", "localStorageService", function(a, b, c, d, e, f, g) {
    ...
    a.claimRestaurant = function() {
        f.claimRestaurant(a.claimForm).then(function(a) {
            ...
            if (a.status < 400) {
                ...
                e.add("success", "Your access code has been sent to your email. Your request-token is " + a.request_token);
            } else {
                j();
            }
        });
    ...
    function j() {
        ...
        a.claimForm.user_id = h.id;
    }
} ]);

"use strict";

rxControllers.controller("requestCtrl", [ "$scope", "$routeParams", "requestSvr", "messageCenterService", function(a, b, c, d) {
    var e = b.restaurantId;
    a.request = {};
    a.submitRequest = function() {
        a.request.restaurant_id = e;
...
angular.module("directive").directive("navTop", navTop).directive("navLeft", navLeft);
...
restaurantService.factory("restaurantSvr", [ "localStorageService", "Restangular", function(a, b) {
    return {
        ...
        getOverviews: function(a, c) {
            var d = "restaurants/" + a + "/stats";
            var e = b.one(d);
            return e.get().then(function(a) {
                c.stats = a.data.items;
                return a.data.items;
            });
        },
...
reviewService.factory("reviewSvr", [ "localStorageService", "Restangular", function(a, b) {
    return {
        getRestaurantReviews: function(a, c) {
            ...
            return d.getList(c).then(function(a) {
...
photoService.factory("photoSvr", [ "localStorageService", "Restangular", function(a, b) {
    return {
        ...
        deleteRestaurantPhotos: function(a) {
            var c = "photos/" + a;
            var d = b.all(c);
            return d.remove(a).then(function(a) {
                return a;
            });
        },
        uploadRestaurantPhoto: function(a, c) {
            var d = "photos/" + a;
            var e = new FormData();
            angular.forEach(c, function(a, b) {
                e.append(b, a);
            });
            b.all(d).withHttpConfig({
