...
rxControllers.config([ "$routeProvider", function(a) {
...
}).controller("indexCtrl", [ "$scope", "$rootScope", "$http", "localStorageService", "$location", "restaurantSvr", "userSvr", "geoLocation", "$routeParams", "$anchorScroll", "searchData", function(a, b, c, d, e, f, g, h, i, j, k) {
    if (i.view) {
        if (i.view == "favorites") {
            a.view = "favorites";
        } else {
    ...
    a.init = function() {
        a.$watch("user", function() {
            ...
            if (a.user && a.view == "favorites") {
                g.getFavorites().then(function(b) {
                    a.favoriteRestaurants = b;
                });
            }
        });
        a.restaurantList = {
        ...
        n();
        a.setAutoLocation(!i.formattedAddress);
        if (i.search) {
            var b = {};
            if (i.formattedAddress) {
                b["formatted-address"] = i.formattedAddress;
            }
            if (i.priceRange) {
                b["price_range"] = i.priceRange;
            }
            if (i.category) {
...
rxControllers.controller("detailCtrl", [ "$scope", "$timeout", "$upload", "localStorageService", "$location", "$routeParams", "restaurantSvr", "geoLocation", "reviewSvr", "overviewSvr", "locationSvr", "photoSvr", "userSvr", "$anchorScroll", "$modal", "$rootScope", function(a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p) {
    var q = null;
    v();
    n();
    ...
    a.checkFavourite = function() {
        m.getFavorites().then(function(b) {
            a.favoriteRestaurants = b;
            a.isFavourite = false;
            angular.forEach(b, function(b) {
                if (b.id == a.restaurantId) {
                    a.isFavourite = true;
                }
            });
        });
    ...
    a.checkFavourite();
    a.favourite = function() {
        if (a.isFavourite) {
            m.removeFavorite(a.restaurantId);
        } else {
            m.addFavorite(a.user.id, a.restaurant.id);
        }
        a.isFavourite = !a.isFavourite;
    };
    var q = s(f.request);
    if (typeof q !== undefined && Object.keys(q).length && a.user) {
        q = o.open({
            templateUrl: q.templateLocation,
            scope: a,
            controller: q.controller,
            windowClass: q.windowClass || ""
        });
        q.opened.then(function() {
            a.showModal = true;
            $(".overlay-main").css("display", "block");
        });
        q.result.then(function(b) {
            a.selected = b;
        }, function() {
    ...
    a.getGraphs = function(c) {
        ...
        angular.forEach(d, function(c) {
            b.getGraphs(a.restaurantId, c.value, a.dates.start.date, a.dates.end.date).then(function(b) {
                if (b.percentile && b.percentile.length > 1) {
                    a.graphs[c.value] = b;
                    a.noGraphs = false;
...
rxControllers.controller("compareCtrl", [ "$scope", "$routeParams", "restaurantSvr", function(a, b, c) {
    ...
    a.graphDuration = a.graphDurations[2];
    a.setGraphDuration = function(b) {
        a.graphDuration = b;
        if (b.value == "") {
    ...
    a.flotDataset = [];
    a.flotColors = [ "#1F2D6D", "#4897C3", "#B75E8F", "#BF1E10", "#D8C746", "#2F9630" ];
    a.flotOptions = {
        grid: {
            hoverable: true,
            clickable: true,
...
rxControllers.controller("widgetsCtrl", [ "$scope", "$routeParams", "restaurantSvr", function(a, b, c) {
    a.options = {
        restaurantId: 0,
        ratingLessThan: 4,
    ...
    a.setRestaurant = function(b) {
        ...
        a.options.restaurantName = b.name;
        c.getRestaurants({
            "id-in": b.id
        }).then(function(b) {
            var c = b.items[0];
            var d = Math.round(c.overview__percentile / 10) / 2;
            if (d < 1) d = 1;
            var e = Math.trunc(d);
            var f = 0;
            if (d - e > 0) {
            ...
            a.options.ratingWhole = e;
            a.options.ratingHalf = f;
            a.options.reviews_str = c.overview__total_reviews == 1 ? "1 review" : c.overview__total_reviews + " reviews";
            a.options.trend_change = c.trend_change;
            a.options.current_trend_change = c.trend_change >= 0 ? "+ " + c.trend_change_abs : "- " + c.trend_change_abs;
            a.options.trend_change_abs = c.trend_change_abs;
            a.options.current_trend_change_color = a.options.trend_change > 0 ? "#5cb85c" : a.options.trend_change < 0 ? "#f0ad4e" : "#92B1BB";
        });
    };
    a.$watch("options", function() {
        var b = "";
        if (a.options.hideOnRatingLessThan) {
            b = ",\n" + "    hideIfRatingLessThan: " + a.options.ratingLessThan;
        }
        var c = "";
        if (a.options.hideOnTrendNegative) {
    ...
    return {
        link: function(b, c, d) {
            ...
            for (e = 0; e < l.length; e++) {
                ...
                m += '><i width="36" height="36" title="Share" class="share-button fa fa-' + l[e].network + '"></a>';
                c.append(m);
            }
        }
    };
    ...
    return {
        getRestaurants: function(c) {
            ...
            return d.getList(merge_objects({
                ...
                return {
                    items: e.expandRestaurantList(a.data[0].items),
                    _meta: a.data[0]._meta
                };
            });
        },
        expandRestaurantList: function(a) {
            for (var b = 0; b < a.length; b++) {
                var c = "";
                for (var d = 0; d <= a[b].price_range; d++) {
                    c = c + "$";
                }
                a[b].price_range_symbol = c;
                var e = JSON.parse(a[b].overview__trend_series);
                var f = [];
                var g = [];
                var h = 0;
                var i = 0;
                ...
                for (var j in e) {
                    ...
                    for (var l in k) {
                        f.push([ l, m - k[l] ]);
                        g.push([ l, a[b].overview__percentile - k[l] ]);
                    }
                }
                a[b].trend_data = [ {
                    key: "Trend",
                ...
                a[b].rating = Math.round(a[b].overview__percentile / 2) / 10;
                a[b].rating_rounded = Math.round(a[b].overview__percentile / 20);
                a[b].overview__percentile_rounded = Math.round(a[b].overview__percentile * 10) / 10;
                a[b].trend_change = 0;
                a[b].trend_change_abs = 0;
                if (e && e.length > 1) {
                    var n, o;
                    for (var p in e[e.length - 1]) {
                        n = e[e.length - 1][p];
                    }
                    for (var q in e[e.length - 2]) {
                        o = e[e.length - 2][q];
                    }
                    a[b].trend_change = Math.round((parseFloat(n) - parseFloat(o)) * 10) / 10;
                    a[b].trend_change_abs = Math.abs(a[b].trend_change);
                }
            }
            return a;
        },
...
userService.factory("userSvr", [ "Restangular", "restaurantSvr", function(a, b) {
    return {
        getFavorites: function() {
            return a.one("users/favourites").withHttpConfig({
                cache: false
            }).get().then(function(a) {
                var c = a.data.items;
                return b.expandRestaurantList(c);
            });
        },
        addFavorite: function(b, c) {
            var d = a.all("favourites");
            var e = $.param({
                user_id: b,
                restaurant_id: c
            });
        ...
        removeFavorite: function(b) {
            var c = a.all("favourites/restaurant/" + b);
            return c.remove().then(function(a) {
                return a;
            });
        }
    };
...
var myApp = angular.module("myApp", [ "ngRoute", "config", "LocalStorageModule", "directive", "restangular", "geoLocation", "overviewService", "restaurantService", "reviewService", "locationService", "photoService", "requestService", "claimService", "userService", "GoogleMaps", "angularFileUpload", "ui.bootstrap", "nvd3ChartDirectives", "easypiechart", "highcharts-ng", "angular-flot", "Controllers", "Services", "MessageCenterModule", "angularFileUpload", "uiGmapgoogle-maps", "cgBusy" ]);
...
myApp.run([ "$rootScope", "$location", "localStorageService", "AuthenticationService", function(a, b, c, d) {
    ...
    a.user = c.get("user");
    a.baseUrl = "http://dev2.reviews-combined.com/";
} ]);
